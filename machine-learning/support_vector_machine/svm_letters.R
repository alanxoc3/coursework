...
# Read the data
data_set <- read.csv("letters.csv", head=TRUE, sep=",")
...
model = svm(formula=letter~., data = data_set_train, kernel="radial", gamma = 0.001, cost = 750)

# Use the model to make a prediction on the test set
# Notice, we are not including the last column here (our target)
prediction = predict(model, data_set_test[,-18])

# Produce a confusion matrix
confusion_matrix = table(pred = prediction, true = data_set_test$letter)

# Calculate the accuracy, by checking the cases that the targets agreed
agreement = prediction == data_set_test$letter
